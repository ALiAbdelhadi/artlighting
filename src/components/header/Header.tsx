          ...
          <NavigationMenu>
            <NavigationMenuList>
              <Link href={"/about-us"} prefetch={false}>
                <NavigationMenuItem className="group inline-flex h-12 w-max items-center justify-center rounded-md px-5 py-3 text-[16px] font-medium transition-colors hover:underline">
                  About Us
                </NavigationMenuItem>
              </Link>
              <NavigationMenuItem>
                ...
                <NavigationMenuContent>
                  <div className="grid w-[800px] grid-cols-3 gap-x-2 p-4 dark:bg-[#1c1a17]">
                    {projectDataForHeader.map((project) => {
                      return (
                        <div
                          className="projectCard relative overflow-hidden"
                          ...
                          <div className="relative figureCard w-full h-full">
                            ...
                            <div className="flex flex-col items-center justify-center mt-4">
                              <h3 className="ProjectDescription  mt-2 mb-2 text-center text-sm tracking-wider">
                                {project.ProjectDescription.slice(0, 24)}...
                              </h3>
                              <Link
                                href={`/all-projects/${project.ProjectId}`}
                                className="projectDate text-sm font-light text-muted-foreground mb-3"
                              >
                                More Details
                              </Link>
                            </div>
                  ...
                  <div className="grid w-[700px] grid-cols-3 gap-2 p-4 justify-items-center dark:bg-[#1c1a17]">
                    <ul className="md:space-y-4  capitalize">
                      <h4 className="font-bold my-2 text-primary text-xl">
                        <Link scroll={true} href={" /category/balcom/indoor"}>
                          indoor
                        </Link>
                      </h4>
                      {IndoorItems.map((IndoorItem) => (
                        <li
                          className="items-center gap-1 font-medium hover:underline  text-[17px]"
                          key={IndoorItem.id}
                        >
                          <Link scroll={true} href={IndoorItem.href}>
                    ...
                    <ul className="md:space-y-4  capitalize">
                      <h4 className="font-bold my-2 text-primary text-xl">
                        <Link scroll={true} href={" /category/balcom/outdoor"}>
                          Outdoor
                        </Link>
                      </h4>
                      {OutdoorItems.map((outdoorItem) => (
                        <li
                          ...
                          key={outdoorItem.id}
                        >
                          <Link scroll={true} href={outdoorItem.href}>
                    ...
                    <ul className="md:space-y-4 capitalize ">
                      ...
                      {ChandelierItems.slice(0, 12).map((ChandelierItem) => (
                        ...
                        >
                          <Link scroll={true} href={ChandelierItem.href}>
